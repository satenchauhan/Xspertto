...
import {SafeAreaView,TextInput, Text, View, StyleSheet,ScrollView,TouchableOpacity,TouchableHighlight, Dimensions} from 'react-native';
import AntDesign from 'react-native-vector-icons/AntDesign';
import MaterialCommunityIcons from 'react-native-vector-icons/MaterialCommunityIcons';
import { addRMStyles } from './addRMStyles';
import Room from '../room/Room';
import TopStatusBar from '../common/TopStatusBar';
...
const AddRoomFrame = (props) => {
    ...
    let ButtonArrayList = buttonArray.map((btn, i) => {
        ...
        if ((i) == indexing) {
            return (
                <TouchableOpacity key={btn.key} bid={btn.bid} style={active === activeNum ? addRMStyles.btnActive : addRMStyles.btnInactive}
                    onPress={() => { showRoom(btn.bid); showActive(activeNum) }}
                    onLayout={(event) => { setLayout(event, last) }}
                >
                    <Text
                        style={active === activeNum ? addRMStyles.ActiveColor : addRMStyles.InactiveColor}>
                        {btn.bName}
                    </Text>
                </TouchableOpacity>
            )
            ...
            return (
                <TouchableOpacity key={btn.key} bid={btn.bid} style={lastActive === activeNum ? addRMStyles.btnActive : addRMStyles.btnInactive}
                    onPress={() => { showRoom(btn.bid); showLastActiveOnClick(activeNum) }}
                    onLayout={(event) => { setLayout(event, last) }}
                >
                    <Text
                        style={lastActive === activeNum ? addRMStyles.ActiveColor : addRMStyles.InactiveColor}>
                        {btn.bName}
                    </Text>
                </TouchableOpacity>
            )
        ...
        if ((i) == indexing) {
            return (
                <TouchableOpacity key={i} bid={item.cbid} style={
                    copyActive === activeNum ? addRMStyles.btnActive : addRMStyles.btnInactive
                    }
                    onPress={() => { showCopyRoom(item.cbid); showCopyActive(activeNum) }}
                    onLayout={(event) => { setLayout(event, activeNum) }}
                >
                    <Text
                        style={copyActive === activeNum ? addRMStyles.ActiveColor : addRMStyles.InactiveColor}>
                        {item.bName}
                    </Text>
                </TouchableOpacity>
            )
            ...
            return (
                <TouchableOpacity key={i} bid={item.cbid} style={
                    lastCopyActive === activeNum ? addRMStyles.btnActive : addRMStyles.btnInactive
                    }
                    onPress={() => { showCopyRoom(item.cbid); showLastCopyActiveOnClick(activeNum) }}
                    onLayout={(event) => { setLayout(event, activeNum) }}
                >
                    <Text
                        style={lastCopyActive === activeNum ? addRMStyles.ActiveColor : addRMStyles.InactiveColor}>
                        {item.bName}
                    </Text>
                </TouchableOpacity>
            )
    ...
    return (
        <SafeAreaView style={[addRMStyles.safeView]} >
            <TopStatusBar  />
            <View style={[addRMStyles.stickySection, {marginBottom:15}]}>
                <View style={addRMStyles.addRoomBtn}>
                    <View style={[addRMStyles.addItem, { marginBottom: 5 }]}>
                        {showInput === true ?
                            <View style={styles.mainInput}>
                                ...
                                <View style={[styles.enterInpt]}>
                                    <TextInput
                                        style={{ flex: 1,paddingVertical:5, fontSize: 18, color:'black', paddingLeft: 10, height:38, }}
                                        type="text"
                                    ...
                                    >
                                        <MaterialCommunityIcons style={{ paddingLeft: 10, marginBottom:5,marginRight:5 }} color="gray" name="close-circle-outline" size={26} />
                                    </TouchableOpacity>
                                </View>
                            </View>
                            : null}
                        {showBtn === true ?
                            <TouchableOpacity onPress={() => { setShowInput(true); setShowBtn(false) }}>
                                {showBtn === true ?
                                    <View style={[addRMStyles.RoomBtn, { backgroundColor: 'white' }]} >
            ...
            <View style={{alignSelf:'flex-end',position:'absolute', right:20, bottom:50,}}>
